...
/**
  ...
  */
object simpleSkyline_Back extends Serializable{
...
//    keywordsReve.foreach(println)
    ...
    /*
      Set(6795 5235, u z)
      Set(6995 6819, r d a)
      Set(3136 2897, h w t h)
    * */
    var resultQuery: RDD[(Int, (Double, Double))] = spatialTextReve(keywordsReve, queryDescribe(0).mkString)
    ...
    //（有问题，因为如果编号为0的网格里面没有数据的话，有可能把结果点删除，所以，要找下标和最小的数据所在的网格编号，再过滤）
...
//    println("test1")
    ...
    dataObject.filter(x => {//利用右上角的点对当前的网格进行过滤，
    ...
    println("data size2: "+dataObject.length)
   // dataObject.foreach(x=>println(x._1,x._2._1,x._2._2.toBuffer))//这俩哪个都可以
    //对过滤后的网格并行进行Skyline处理
    val localSkyline: Array[(Int, Array[(Double)])] = sc.parallelize(dataObject,ParNumber).groupByKey().mapPartitions(iter=>iter.flatMap(y=>spatialSkyline(Dim,y._2.toArray))).collectAsMap().toArray
    val Skyline = spatialSkyline(Dim,localSkyline).map(obj=>obj._1).toList//.foreach(println)
