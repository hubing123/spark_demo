...
/**
  ...
  */
object SimpleSkyline_Compare extends Serializable{
...
val  start = System.currentTimeMillis()
    ...
    val queryKeywords: RDD[Seq[String]] = queryFile.map(line => {
      val obj = line.split(",")
      val keywords = obj(1).split(" ").drop(1)
      keywords
    }.toSeq)

    val keywordString=queryKeywords.flatMap(x=>x ).collect().toSet.foldLeft("")(_ + " " + _).trim//关键字集合

    val queryDescribe:Array[String] = queryFile.map(line=>{//查询点只留下坐标
      val cor = line.split(",")(0)
      cor+","+keywordString
    }).collect()
    val Dim = queryDescribe.length

//    queryDescribe.foreach(println)
    var resultQuery: RDD[(Int, (Double, Double))] = spatialTextReve(keywordsReve, queryDescribe(0)+","+keywordString(0))
    if (queryDescribe.length > 1) {
    ...
    val obj_query_Result = resultQuery.groupByKey().filter(line => line._2.size == queryDescribe.length)//经过计算之后有的对象和某个查询点的相关性为0，则直接过滤掉，
//    obj_query_Result.foreach(println) //(8,CompactBuffer((7953.51,1.7), (3663.85,1.7), (3368.65,1.7)))其中8是空间对象的编号，后边分别是到每个查询点的距离和文本相关性
    ...
    println("data size1: "+obj_query_Result.collect().length)

    val middle = System.currentTimeMillis()
    println("middle time:"+(middle-start))
    ...
    val dataObject: Array[(Int, (Int, Array[Double]))] = obj_query_Result.map(x => {
      ...
      val corValue = x._2.map(y => y._1 * 0.5 + y._2 * 0.5).toArray
...
//    dataObject.foreach(x=>println(x._1,x._2._1,x._2._2.toBuffer))//这俩哪个都可以
    ...
    val data:Array[(Int, (Int, Array[Double]))] = dataObject.filter(x => {//利用右上角的点对当前的网格进行过滤，
    ...
    println("grids  filter later")
    println("data size2: "+data.length)
    val localSkyline: Array[(Int, Array[(Double)])] = sc.parallelize(data, ParNumber).groupByKey().mapPartitions(iter =>
      iter.flatMap(y => spatialSkyline(Dim, y._2.toArray))).collectAsMap().toArray


    val Skyline = spatialSkyline(Dim,localSkyline).map(obj=>obj._1).toList

    println(Skyline.length)
    val  end = System.currentTimeMillis()
    print("cost times："+(end-start))//返回毫秒数，1秒(s)=1000毫秒(ms)
    sc.stop()
  }
  ...
  def spatialSkyline(Dim: Int, dataObject: Array[(Int, Array[(Double)])]): Array[(Int, Array[(Double)])] = {
    ...
    val Skyline: Array[Array[(Double)]] = dominateCompute(Dim, Dominator, arr) //.map(x=>findID(x,dataObject)).toList.distinct

    val SkylineID: Array[Int] = Skyline.map(x => findID(x, dataObject)) //.toList.distinct
